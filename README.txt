...
the image: x,y indicate how many cells to split the image into, and i,j are indices
that identify which cell to render.  This is to allow an image to be rendered by
multiple independent Raytracer.exe processes.  As an example, if you want to render
using 4 processes, you can do:
...
together to form the final image.  This was done using ImgAdder.exe, which was a
small program written using theh lodepng class.


Alternatively, Raytracer.exe can render the image in multiple threads, by adding
