...
void ExampleScene::initializeAs5UniformScene()
{
    rtClear();
    ...
    ////global settings
    rtCamera(/*eye*/STPoint3(100.f, 130.f, 150.f),/*up*/STVector3(0.f, 1.f, 0.f),/*lookat*/STPoint3(0.f, 0.f, 0.f),/*fov*/45.f,/*aspect*/1.f);
    rtOutput(/*width*/512,/*height*/512,/*path*/"./as5_uniform_4.png");
    rtBounceDepth(1);
    //rtUseShadow(false);
    rtShadowBias(1e-4f);
    //rtSampleRate(4);
    ...
    ////lighting
    rtAmbientLight(STColor3f(.1f, .1f, .1f));
    rtPointLight(STPoint3(0.f, 0.f, 90.f), STColor3f(1.f, 1.f, 1.f));

    ////objects
    int counts[3] = { 15, 15, 15 };
...
void ExampleScene::initializeAs5NonUniformScene() {
    rtClear();
    ...
    ////global settings
    rtCamera(/*eye*/STPoint3(100.f, 130.f, 150.f),/*up*/STVector3(0.f, 1.f, 0.f),/*lookat*/STPoint3(0.f, 0.f, 0.f),/*fov*/45.f,/*aspect*/1.f);
    rtOutput(/*width*/512,/*height*/512,/*path*/"./as5_nonuniform_2.png");
    rtBounceDepth(1);
    //rtUseShadow(false);
    rtShadowBias(1e-4f);
    //rtSampleRate(4);
        ...
        for (int j = 0; j<counts[1]; j++){
            for (int k = 0; k<counts[2]; k++){
                ...
                for (int i = 0; i < 3; i++) {
                    ...
                    int sign = c > 0.f ? 1 : -1;
                    c *= sign;
                    perturb.Component(i) =  sign * pow(c, 0.25f) * .5f + .5f;
                }
                //STPoint3 center = min_corner + STVector3(dx.Component(0)*(float)(i + .5f + (perturb.Component(0) - .5f)*.2f), dx.Component(1)*(float)(j + .5f + (perturb.Component(1) - .5f)*.2f), dx.Component(2)*(float)(k + .5f + (perturb.Component(2) - .5f)*.2f));
                STPoint3 center = min_corner + STVector3(perturb.Component(0) * size[0],
                    perturb.Component(1) * size[1],
