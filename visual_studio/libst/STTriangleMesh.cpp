...
bool STTriangleMesh::UpdateGeometry()
{
    ...
    if(mVertices.size()>0){
        ...
        for(unsigned int i=1;i<mVertices.size();i++){
            mBoundingBoxMin = mBoundingBoxMin.cwiseMin(mVertices[i].pt);  //STPoint3::Min(mBoundingBoxMin,mVertices[i]->pt);
            mBoundingBoxMax = mBoundingBoxMax.cwiseMax(mVertices[i].pt);   //STPoint3::Max(mBoundingBoxMax,mVertices[i]->pt);
        }
    }
    else{
    ...
    for(unsigned int mesh_id=0; mesh_id<shapes.size(); mesh_id++)
    {
        ...
        if(mesh.normals.size()>0){
            for(unsigned int normal_id=0; normal_id<mesh.normals.size()/3; normal_id++)
                stmesh->mNormals.push_back(STVector3(mesh.normals[normal_id*3],     // must be push_back to invoke STVector3(x,y,z) macro
        ...
        if(mesh.texcoords.size()>0){
            for(unsigned int texpos_id=0; texpos_id<mesh.texcoords.size()/2; texpos_id++)
                stmesh->mTexPos.emplace_back(mesh.texcoords[texpos_id*2],
...
std::pair<STPoint3,STPoint3> STTriangleMesh::GetBoundingBox(const std::vector<STTriangleMesh*>& input_meshes)
{
    if(input_meshes.size()>0){
        ...
        for(unsigned int i = 0; i<input_meshes.size(); i++){
            boundingMin = boundingMin.cwiseMin(input_meshes[i]->mBoundingBoxMin);   //STPoint3::Min(boundingMin,input_meshes[i]->mBoundingBoxMin);
            boundingMax = boundingMax.cwiseMin(input_meshes[i]->mBoundingBoxMax);   //STPoint3::Max(boundingMax,input_meshes[i]->mBoundingBoxMax);
        }
